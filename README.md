...
The dataset is not redistributed in this repository due to copyright. Use `collect_music.py` to recreate it, or substitute your own labeled audio.
...
Training artifacts (confusion matrix, training curves, Grad-CAM maps, per-class reports) are in `Training_Results/`.
...
├── Training_Results/           # All outputs from the final training run
│   ├── best_model/config.yaml  # Best hyperparameter configuration
│   ├── model_weights/          # Final model weights + scaler
│   ├── results/                # Classification report + training log CSV
│   ├── plots/                  # Confusion matrix + training curves
│   └── xai/                    # Grad-CAM visualizations per genre
├── checkpoints/
...
### Full training (custom config)

Edit `Training_Results/best_model/config.yaml` to change hyperparameters, then run the training entry point in `src/train.py` directly. Make sure `data_csv` and `audio_dir` in the config point to your local dataset.
...
Grad-CAM heatmaps are generated by `src/explain.py`, targeting `conv_block6` of the CNN14 backbone. Each heatmap shows which time-frequency regions of the log-mel spectrogram most influenced the model's prediction. Pre-generated examples for all five genres are in `Training_Results/xai/individual_examples/`.
...
| **Training Results** | `Training_Results/` — classification report, training log (CSV), confusion matrix (normalized), training curves, and per-genre Grad-CAM visualizations. |
